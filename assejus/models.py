        ...
        if self.tipo_poderes in ['especificos', 'ambos']:
            ...
            if (self.processo_especifico or self.numero_processo_especifico):
                if self.poderes_especificos and self.poderes_especificos.strip():
                    # Usar poderes específicos personalizados se existirem
                    poderes_especificos_limpo = self._limpar_html(self.poderes_especificos)
                    texto_padrao += f"""<b>PODERES ESPECÍFICOS:</b> {poderes_especificos_limpo}"""
                else:
                    # Usar texto padrão dos poderes específicos referente ao processo selecionado
                    if self.processo_especifico:
                        # Se há processo específico cadastrado, usar formato simples
...
<<<<<<< HEAD
    def _limpar_html(self, texto):
        ...
        # Remover tags vazias problemáticas
...
class ModeloPoderes(models.Model):
    """
    Modelo para armazenar templates de poderes gerais e específicos
    """
    CATEGORIA_CHOICES = [
        ('geral', 'Geral'),
        ('civil', 'Civil'),
    ...
    publico = models.BooleanField(default=False, verbose_name='Modelo Público')
