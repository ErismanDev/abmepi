...
<<<<<<< HEAD
    ...
    # Campo personalizado para número do processo com autocomplete
    numero_processo_nome = forms.CharField(
        max_length=50,
        required=False,
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'id': 'id_numero_processo_nome',
...
<<<<<<< HEAD
        ...
        if self.instance and self.instance.pk:
            if self.instance.parte_cliente:
                self.fields['parte_cliente_nome'].initial = str(self.instance.parte_cliente)
...
<<<<<<< HEAD
        ...
        numero_processo_id = cleaned_data.get('numero_processo')
        
        # Validação do campo parte_cliente
        if not parte_cliente_id and parte_cliente_nome:
            from associados.models import Associado
            try:
                # Tentar encontrar o associado pelo nome
                associado = Associado.objects.get(nome=parte_cliente_nome)
                cleaned_data['parte_cliente'] = associado
            except Associado.DoesNotExist:
                self.add_error('parte_cliente_nome', 'Associado não encontrado. Selecione da lista de sugestões.')
            except Associado.MultipleObjectsReturned:
                self.add_error('parte_cliente_nome', 'Múltiplos associados encontrados. Selecione da lista de sugestões.')
        elif not parte_cliente_id:
            self.add_error('parte_cliente_nome', 'Selecione um associado.')
        
        # Validação do campo numero_processo
        if not numero_processo_id and numero_processo_nome:
            from .models import ProcessoJuridico
            try:
                # Tentar encontrar o processo pelo número
                processo = ProcessoJuridico.objects.get(numero_processo=numero_processo_nome)
                cleaned_data['numero_processo'] = processo.numero_processo
            except ProcessoJuridico.DoesNotExist:
                # Se não encontrar, usar o número digitado como novo processo
                cleaned_data['numero_processo'] = numero_processo_nome
            except ProcessoJuridico.MultipleObjectsReturned:
...
<<<<<<< HEAD
            'poderes_gerais': forms.Textarea(attrs={
                ...
                'rows': 8,
                'placeholder': 'Descreva os poderes gerais da procuração...'
            }),
            'poderes_especificos': forms.Textarea(attrs={
                'class': 'form-control editor-texto',
                'id': 'id_poderes_especificos',
                'rows': 8,
...
<<<<<<< HEAD
        ...
        # Validação do campo outorgante
        if not outorgante_id and outorgante_nome:
            from associados.models import Associado
            try:
                # Tentar encontrar o associado pelo nome
                associado = Associado.objects.get(nome=outorgante_nome)
                cleaned_data['outorgante'] = associado
            except Associado.DoesNotExist:
                self.add_error('outorgante_nome', 'Associado não encontrado. Selecione da lista de sugestões.')
            except Associado.MultipleObjectsReturned:
