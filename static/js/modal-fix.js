...
<<<<<<< HEAD
        ...
        if (event.message && event.message.includes('openFeaturedNewsModal is not defined')) {
            console.warn('⚠️ Erro de openFeaturedNewsModal interceptado:', event.message);
...
function preserveNewsModalFunctions() {
    ...
    // Garantir que openFeaturedNewsModal não seja perdida
    if (typeof window.openFeaturedNewsModal !== 'function') {
        console.log('⚠️ openFeaturedNewsModal não encontrada, verificando se deve ser restaurada...');
        
        // Aguardar um pouco para outros scripts carregarem
        setTimeout(() => {
            if (typeof window.openFeaturedNewsModal !== 'function') {
                console.warn('❌ openFeaturedNewsModal ainda não disponível após timeout');
                
                // Tentar definir uma função de fallback
                window.openFeaturedNewsModal = function(title, category, priority, date, excerpt, image) {
                    console.log('🔧 Função openFeaturedNewsModal de fallback chamada com:', {title, category, priority, date, excerpt, image});
                    
                    // Verificar se a função real foi carregada enquanto isso
                    if (window.openFeaturedNewsModal !== arguments.callee) {
                        console.log('✅ Função real encontrada, redirecionando...');
                        return window.openFeaturedNewsModal(title, category, priority, date, excerpt, image);
                    }
                    
                    console.warn('⚠️ Função openFeaturedNewsModal não está disponível. Verifique se o script da página foi carregado.');
                    if (typeof showErrorMessage === 'function') {
                        showErrorMessage('Erro: Função de modal de notícias não está disponível. Recarregue a página e tente novamente.');
                    } else {
                        console.error('❌ Função de modal de notícias não está disponível');
                    }
                };
                
                console.log('✅ Função de fallback para openFeaturedNewsModal criada');
            }
        }, 1000);
    } else {
        console.log('✅ openFeaturedNewsModal já está disponível');
    }
    
    // Garantir que openNewsModal não seja perdida
    if (typeof window.openNewsModal !== 'function') {
        console.log('⚠️ openNewsModal não encontrada, verificando se deve ser restaurada...');
        
        // Aguardar um pouco para outros scripts carregarem
        setTimeout(() => {
            if (typeof window.openNewsModal !== 'function') {
                console.warn('❌ openNewsModal ainda não disponível após timeout');
                
                // Tentar definir uma função de fallback
                window.openNewsModal = function(newsData) {
                    console.log('🔧 Função openNewsModal de fallback chamada com:', newsData);
                    
                    // Verificar se a função real foi carregada enquanto isso
                    if (window.openNewsModal !== arguments.callee) {
                        console.log('✅ Função real encontrada, redirecionando...');
                        return window.openNewsModal(newsData);
                    }
                    
                    console.warn('⚠️ Função openNewsModal não está disponível. Verifique se o script da página foi carregado.');
                    if (typeof showErrorMessage === 'function') {
                        showErrorMessage('Erro: Função de modal de notícias não está disponível. Recarregue a página e tente novamente.');
                    } else {
                        console.error('❌ Função de modal de notícias não está disponível');
                ...
                console.log('✅ Função de fallback para openNewsModal criada');
            }
        }, 1000);
    } else {
        console.log('✅ openNewsModal já está disponível');
    }
}
